...
if(isset($_POST['add_r']))
{
  ...
  $pick=$db->connection->query("SELECT employee_code, name, category, sum(paye)  total_paye from sliprecord where  year='$year' GROUP BY employee_code  " );
  if($pick->num_rows > 0)
  {
...
while ($row=$pick->fetch_array()) {
  ?>
          <tr>
            ...
            <td><?php echo number_format($row['total_paye'],2)  ?></td>
