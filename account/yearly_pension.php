...
if(isset($_POST['add_r']))
{
  ...
  $pick=$db->connection->query("SELECT employee_code, name, category, sum(pension)  total_pension from sliprecord where  year='$year' GROUP BY employee_code  " );
  if($pick->num_rows > 0)
  {
...
while ($row=$pick->fetch_array()) {
  ?>
          <tr>
            ...
            <td><?php echo number_format($row['total_pension'],2)  ?></td>
