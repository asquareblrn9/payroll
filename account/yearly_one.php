...
<div class="col-md-3">
  ...
  <div class="col-md-3">
    <?php $sql=$db->connection->query("SELECT employee_code from employee ") ?>
    <select name="ids" class="form-control">
      <?php
        while ($rows=$sql->fetch_assoc()) {
          echo "<option value=".$rows['employee_code'].">".$rows['employee_code']."</option>";
        }
...
if(isset($_POST['add_r']))
{
  ...
  $ids = $con->escape_string($_POST['ids']);
  $pick=$db->connection->query("SELECT * from sliprecord where  year='$year' and employee_code = '$ids' GROUP BY month ORDER BY id  ASC" );
  if($pick->num_rows > 0)
  {
